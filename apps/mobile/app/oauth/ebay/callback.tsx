...
export default function EbayOAuthCallbackScreen() {
  ...
  const [message, setMessage] = useState("Completing eBay connection...");

  useEffect(() => {
    if (error) {
      setStatus("error");
      setMessage(error_description ?? error ?? "eBay authorization was denied.");
      return;
    }
  ...
  async function completeOAuth(oauthCode: string, oauthState: string) {
    try {
      ...
      setStatus("success");
      setMessage("eBay connected! Redirecting...");
      setTimeout(() => {
        router.replace("/integrations");
      }, 1200);
    } catch (err) {
      setStatus("error");
      setMessage((err as Error).message ?? "Failed to complete eBay OAuth.");
    }
  }
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.backgroundWash }]}>
      <Text style={[styles.title, { color: colors.text }]}>eBay OAuth</Text>
